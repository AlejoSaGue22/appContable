...
export class ReciboCajaFormsPageComponent implements OnInit {
  ...
  headTitle = signal<HeaderInput>({
    ...
    slog: 'Registrar abonos a facturas de clientes o ingresos por otros conceptos directos',
  });

  form!: FormGroup;
  modoCobro = signal<'factura' | 'otros'>('factura');
  ...
  // Facturas pendientes de cobro para el cliente seleccionado
  ...
  ngOnInit(): void {
    ...
    // Reaccionar al cambio de cliente para cargar facturas
    this.form.get('terceroId')?.valueChanges.subscribe(clienteId => {
      if (clienteId && this.modoCobro() === 'factura') {
  ...
  onClienteSeleccionado(cliente: any): void {
    this.form.patchValue({
      ...
      clienteSearch: cliente.nombreDisplay
    });
    this.clienteSearch.set(cliente.nombreDisplay);
  }

  cambiarModo(modo: 'factura' | 'otros'): void {
    this.modoCobro.set(modo);
    // Limpiar arrays
    ...
    const clienteCtrl = this.form.get('terceroId');
    if (modo === 'otros') {
      clienteCtrl?.clearValidators(); // Opcional para otros ingresos
      this.agregarConceptoLinea();
    } else {
      clienteCtrl?.setValidators([Validators.required]);
      const clienteId = clienteCtrl?.value;
      if (clienteId) {
        this.cargarFacturasPendientes(clienteId);
      }
    }
  ...
  cargarCatalogos(): void {
    ...
    this.clientesService.getClientes({ limit: 100, offset: 0 }).subscribe((res: any) => {
      const mapped = (res.clientes || []).map((c: any) => {
        const name = c.razonSocial?.trim() ? c.razonSocial : `${c.nombre || ''} ${c.apellido || ''}`.trim();
        return {
          ...c,
          nombreDisplay: name
        };
      });
      this.clientes.set(mapped);
    });

    // 2. Cuentas Bancarias
    this.pagosService.getCuentasBancarias().subscribe(res => {
    ...
    this.cuentasService.getCuentasContables().subscribe(res => {
      this.cuentasContables.set(res.filter(c => c.aceptaMovimiento && c.isActive));
    });
  ...
  cargarFacturasPendientes(clienteId: string): void {
    this.facturasLoading.set(true);
    while (this.detallesFormArray.length) this.detallesFormArray.removeAt(0);

    this.pagosService.getFacturasPendientesCliente(clienteId).subscribe({
      next: (facts: any[]) => {
        ...
        facts.forEach((f: any) => {
          this.detallesFormArray.push(this.fb.group({
            facturaId: [f.id],
            comprobante: [f.comprobante_completo],
            total: [f.total],
  ...
  // Métodos para Option 1: Cobro a facturas
  ...
  get totalGeneral(): number {
    return this.modoCobro() === 'factura' ? this.sumarTotalAbonos() : this.sumarTotalOtros();
  }
  ...
  guardar(): void {
    ...
    if (this.modoCobro() === 'factura') {
      ...
      this.pagosService.registrarCobroMultiple(payload).subscribe({
        next: () => {
          this.submitting.set(false);
          this.router.navigate(['/panel/pagos/cxc']);
        },
        error: (err: any) => {
          this.submitting.set(false);
          this.notificationService.error(err.error?.message || 'Ocurrió un error al registrar el recibo de caja.', 'Error');
        }
      });
      ...
      this.pagosService.registrarOtrosIngresos(payload).subscribe({
        next: () => {
          this.submitting.set(false);
          this.router.navigate(['/panel/pagos/cxc']);
        },
        error: (err: any) => {
          this.submitting.set(false);
          this.notificationService.error(err.error?.message || 'Ocurrió un error al registrar otros ingresos.', 'Error');
        }
      });
    }
  }
