            ...
            <tbody class="divide-y divide-slate-100">
                @for (emp of empleados(); track emp.id) {
                    <tr class="hover:bg-slate-50 transition-colors">
                        ...
                        <td class="px-4 py-3">
                            <a [routerLink]="['/panel/nomina/empleados', emp.id, 'detalle']"
                        ...
                        <td class="px-4 py-3 text-right space-x-2">
                            <a [routerLink]="['/panel/nomina/empleados', emp.id, 'detalle']"
                                class="text-blue-600 hover:text-blue-800 text-xs font-bold">Ver</a>
                            <a [routerLink]="['/panel/nomina/empleados', emp.id, 'editar']"
                                class="text-amber-600 hover:text-amber-800 text-xs font-bold">Editar</a>
                            <button (click)="delete.emit(emp.id)"
                                class="text-red-600 hover:text-red-800 text-xs font-bold">Eliminar</button>
                        </td>
                    </tr>
                } @empty {
                    <tr>
