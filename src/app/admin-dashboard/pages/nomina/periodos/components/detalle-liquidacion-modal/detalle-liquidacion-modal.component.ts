...
@Component({
  ...
  template: `
    ...
    >
      <div
        class="bg-white rounded-2xl shadow-2xl border border-slate-200 w-full max-w-4xl overflow-hidden max-h-[90vh] flex flex-col"
      >
        <div
        ...
        <div
          class="px-6 py-3 bg-slate-50 border-b border-slate-100 flex-shrink-0"
        >
          <div class="grid grid-cols-4 gap-4 text-center text-sm">
            <div>
              ...
              <p class="text-lg font-bold text-slate-700">
                \${{ periodo().totalDevengado | number: '1.0-0' }}
              </p>
            </div>
            <div>
              <p class="text-xs text-slate-500 uppercase font-bold">
              ...
              <p class="text-lg font-bold text-red-600">
                \${{ periodo().totalDeducciones | number: '1.0-0' }}
              </p>
            </div>
            <div>
              <p class="text-xs text-slate-500 uppercase font-bold">
              ...
              <p class="text-lg font-bold text-green-600">
                \${{ periodo().totalNeto | number: '1.0-0' }}
              </p>
            </div>
            <div>
              <p class="text-xs text-slate-500 uppercase font-bold">
              ...
              <p class="text-lg font-bold text-blue-600">
                \${{ periodo().totalCostoEmpresa | number: '1.0-0' }}
              </p>
            </div>
          </div>
        </div>
        ...
        <div class="overflow-y-auto flex-1 p-6">
              ...
              @for (liq of liquidaciones(); track liq.id) {
                <tr class="hover:bg-slate-50">
                  <td class="px-3 py-2 font-medium">
                    {{ liq.empleado?.primerNombre }}
                    {{ liq.empleado?.primerApellido }}
                  </td>
                  <td class="px-3 py-2 font-mono text-right">
                    \${{ liq.totalDevengado | number: '1.0-0' }}
                  </td>
                  <td class="px-3 py-2 font-mono text-right">
                    \${{ liq.saludEmpleado | number: '1.0-0' }}
                  </td>
                  <td class="px-3 py-2 font-mono text-right">
                    \${{ liq.pensionEmpleado | number: '1.0-0' }}
                  </td>
                  <td class="px-3 py-2 font-mono text-right">
                    \${{ liq.retencionFuente | number: '1.0-0' }}
                  </td>
                  <td
                    class="px-3 py-2 font-mono text-right font-bold text-green-600"
                  >
                    \${{ liq.netoPagar | number: '1.0-0' }}
                  </td>
                  <td class="px-3 py-2 font-mono text-right text-slate-500">
                    \${{ liq.ibc | number: '1.0-0' }}
                  </td>
                  <td class="px-3 py-2 font-mono text-right text-blue-600">
                    \${{
                      liq.totalDevengado +
                        liq.totalAportes +
                        liq.totalProvisiones | number: '1.0-0'
                    }}
                  </td>
                </tr>
              }
          ...
          @if (periodo().dianEstado && periodo().dianEstado !== 'NO_ENVIADA') {
            <div class="mt-6 border-t border-slate-200 pt-4">
              <h4 class="text-sm font-bold text-slate-600 uppercase mb-3">
              ...
              <div class="grid grid-cols-3 gap-4 text-sm">
                <div>
                  ...
                  <p
                    class="font-bold"
                    [class.text-green-600]="periodo().dianEstado === 'ACEPTADA'"
                    [class.text-yellow-600]="periodo().dianEstado === 'ENVIADA'"
                  >
                    {{ periodo().dianEstado }}
                  </p>
                </div>
                  ...
                  @for (p of pagos(); track p.id) {
                    <tr class="hover:bg-slate-50">
                      ...
                      <td class="px-3 py-2 font-mono text-xs">
                        {{ p.cuentaCodigoContable }}
                      </td>
                      <td class="px-3 py-2 text-xs">
                        {{ p.numeroComprobante || '-' }}
                      </td>
                      <td
                        class="px-3 py-2 font-mono text-right text-green-600 font-bold"
                      >
                        \${{ p.valor | number: '1.0-0' }}
                      </td>
