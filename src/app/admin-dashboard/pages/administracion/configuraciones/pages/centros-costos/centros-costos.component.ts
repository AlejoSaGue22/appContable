...
export class CentrosCostosComponent implements OnInit {
  ...
  save() {
    ...
    if (this.isEditing() && this.currentId()) {
      this.centrosCostosService
        .updateCentroCosto(this.currentId()!, formValue)
        .subscribe({
          next: () => {
          ...
          error: (err) => {
            this.toastService.error(
              err.error?.message || 'Error al actualizar el centro de costo',
            );
          },
        });
    } else {
      this.centrosCostosService.createCentroCosto(formValue).subscribe({
        ...
        error: (err) => {
          this.toastService.error(
            err.error?.message || 'Error al crear el centro de costo',
          );
        },
      });
    }
